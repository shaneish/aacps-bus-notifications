...
def format_notification(row, school, col_map):
    ...
    return "\n\n".join(["Affected Bus:", bus, time_slot, school, sub_bus, impact]).strip()
...
def notify_users_map(raw_data, current_dir, configs, logging=True):
    ...
    if logging:
        ...
        log_file = logs_dir / f"{datetime.now().strftime('%d-%m-%Y-%H-%M-%S')}-logs.html"
        with open(log_file, "w") as log:
            log.write(raw_data.strip().replace("\r", ""))

        # delete old logs past threshold
        logs = [logs_dir / log for log in os.listdir(logs_dir) if log.split(".")[-1] == "html"]
    ...
    if valid_data:
        ...
        for phone_num, bus_num, school, always_notify in get_number_iterator(
            ...
            phone_num, text = create_notification(phone_num, bus_num, school, always_notify, message_map)
        ...
        text_map[configs["debug"]["to_phone"]] = [f"Error: Table does not have proper schema.\n\n{col_map.keys()}"]
...
def filter_texts(raw_texts_to_send, logs_dir, configs, compare=False):
    ...
    if os.path.exists(old_texts_location) and compare:
        ...
        for phone_num in set(raw_texts_to_send.keys()).union(set(previous_texts_sent.keys())):
            new_texts = set(raw_texts_to_send.get(phone_num, []))
            old_texts = set(previous_texts_sent.get(phone_num, []))
            if set(texts.lower() for texts in new_texts) != set(texts.lower() for texts in old_texts):
                new_bus_shortage = list(new_texts - old_texts)
                old_bus_reversal = old_texts - new_texts
                old_bus_reversal = list(map(old_bus_reversal, reverse_notification)) if old_bus_reversal else []
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--compare", action="store_true", help="Compares the current schedule to the previous one to determine messages to send")
    ...
    raw_texts_to_send, always_raw_texts = notify_users_map(raw_data, current_dir, configs, args.log)
